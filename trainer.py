...
class Trainer(object):
    ...
    def train(self,train_sampler=None,args=None):
        ...
        auroc, auprc, f1, sensitivity, specificity, accuracy, test_loss,  \
            thred_optim, precision, atts, att1s, att2s= self.test(dataloader="test", DDP=args.DDP)
    ...
    def test(self, dataloader="test", DDP=None):
        ...
        with torch.no_grad():
            ...
            for i, (v_d, v_p, labels) in enumerate(data_loader):
                v_d, v_p, labels = v_d.to(self.device), v_p.to(self.device), labels.float().to(self.device)
                if dataloader == "val":
                    v_d, v_p, score, att, att1, att2= self.model(v_d, v_p, mode='val')
                elif dataloader == "test":
                    checkpoint = torch.load(os.path.join(self.output_dir, f"best_model.pth"), map_location=self.device)
                    self.model.load_state_dict(checkpoint["MODEL_STATE"])
                    if DDP:
                        dist.barrier()  #等待所有的进程完成  #DDP
                    v_d, v_p, score, att, att1, att2 = self.model(v_d, v_p, mode='test')
                    if i==0:
                        if att == None:
                            atts = 0
                        else:
                            atts = np.array(att.cpu())
                        if att2 == None:
                            att2s = 0
                        else:
        ...
        if dataloader == "test":
            ...
            return auroc, auprc, np.max(f1[5:]), sensitivity, specificity, accuracy, test_loss, \
                   thred_optim, precision1, atts, att1s, att2s
        else:
